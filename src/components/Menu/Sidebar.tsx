...
const Sidebar: FC<SidebarProps> = ({ onClickOutside }) => (
  <nav className="absolute top-full left-0 transition-all">
    ...
    <ul className="scrollY relative top-8 -left-10 flex h-screen max-h-[calc(100vh-6.7rem)] w-screen flex-col items-start justify-start gap-12 border-r-2 border-t-2 border-[#121212] bg-white p-6 pl-12 dark:border-white dark:bg-[#121212] sm:w-96">
      <SidebarItem href="/about" label="About" icon={<UserIcon />} />

      <SidebarItem expanded label="Categories" icon={<StackIcon />}>
        <ul className="my-4 ml-12 w-full space-y-4">
